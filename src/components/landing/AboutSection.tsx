...
const AboutSection = () => {
  ...
  return (
    <section id="about" className="py-12 sm:py-16 md:py-24 bg-background relative overflow-hidden">
      ...
      <div className="absolute top-20 left-10 w-48 sm:w-72 h-48 sm:h-72 bg-primary/5 rounded-full blur-3xl animate-float hidden sm:block" />
      <div className="absolute bottom-20 right-10 w-64 sm:w-96 h-64 sm:h-96 bg-primary/5 rounded-full blur-3xl animate-float hidden sm:block" style={{ animationDelay: "1.5s" }} />
      
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 md:gap-16 items-center">
          {/* Left Content */}
          <div className="about-animate slide-left-on-scroll">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-3">
              {["Fact-Checked", "Unbiased", "24/7 Coverage", "Global Reach"].map((badge, i) => (
                <span
                  key={badge}
                  className="about-animate px-3 sm:px-4 py-1.5 sm:py-2 bg-secondary rounded-full text-xs sm:text-sm font-medium text-foreground border border-border hover:border-primary/30 hover:bg-primary/5 transition-all duration-300 cursor-default scale-up-on-scroll"
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6">
            {stats.map((stat, index) => (
              <div
                key={stat.label}
                className="about-animate group relative bg-card rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 border border-border hover:border-primary/30 transition-all duration-500 scale-up-on-scroll overflow-hidden"
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary/10 rounded-lg sm:rounded-xl flex items-center justify-center mb-3 sm:mb-4 group-hover:scale-110 group-hover:bg-primary transition-all duration-300">
                    <stat.icon className="w-5 h-5 sm:w-6 sm:h-6 text-primary group-hover:text-primary-foreground transition-colors" />
                  </div>
                  <div className="font-display text-2xl sm:text-3xl md:text-4xl font-bold text-foreground mb-1 group-hover:text-primary transition-colors">
                    {stat.value}
                  </div>
